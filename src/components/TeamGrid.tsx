...
interface PartnerBioDetail {
  name: string
  role: string
  bio: string[]
  expertise: string[]
  fourExpertise: string[]
}

const PARTNER_DETAILS: Record<string, PartnerBioDetail> = {
  "ritesh arora": {
    name: "CA Ritesh Arora",
    role: "Partner",
    bio: [
      "CA Ritesh Arora is a Partner at Ritesh Arora & Associates and specialises in indirect taxation, GST advisory, litigation and business consulting.",
      "With over a decade of professional experience, he advises businesses across diverse industries on complex tax matters, regulatory compliance and strategic business issues. He regularly represents clients before tax authorities and appellate forums, assisting them in resolving intricate tax disputes through practical and commercially viable solutions.",
      "He is also the author of GST Gavel – A Litigation Guide and actively contributes to the profession through technical sessions, professional publications and knowledge-sharing initiatives."
    ],
    expertise: [
  ...
  "diksha arora": {
    ...
    role: "Partner",
    bio: [
      "CA Diksha Arora is a Partner at Ritesh Arora & Associates with extensive experience in audit & assurance, financial reporting, accounting and regulatory compliance.",
      "She works closely with businesses to strengthen governance, improve financial processes and ensure timely statutory compliance. Her practical and client-focused approach enables organisations to build efficient financial systems while maintaining the highest standards of professional integrity.",
      "She is currently serving as the Chairperson of the ICAI Amritsar Branch (2026-27)."
    ],
    expertise: [
      "Audit & Assurance",
...
export function TeamGrid({ members }: TeamGridProps) {
  ...
  const [selectedPartner, setSelectedPartner] = useState<PartnerBioDetail | null>(null)
  ...
  const handleMemberClick = (member: TeamMember) => {
    ...
    const lookupKey = member.name.toLowerCase().replace(/^ca\s+/i, '').trim()
    const detail = PARTNER_DETAILS[lookupKey]
    if (detail) {
      setSelectedPartner(detail)
      setSelectedMember(member)
    } else {
      // Fallback for non-partners or other team members
      const fallbackDetail: PartnerBioDetail = {
        name: member.name,
        role: member.role,
        bio: ["Chartered Accountant and key professional at Ritesh Arora & Associates committed to client success and service excellence."],
        expertise: ["Taxation", "Accounting", "Corporate Compliance", "Financial Advisory"],
        fourExpertise: ["Taxation", "Accounting", "Compliance", "Advisory"]
      }
      setSelectedPartner(fallbackDetail)
      setSelectedMember(member)
    }
  }
  ...
  return (
    <>
      ...
      <div className="grid md:grid-cols-3 gap-8 max-w-7xl mx-auto px-4">
        {members.map((m) => {
          const lookupKey = m.name.toLowerCase().replace(/^ca\s+/i, '').trim()
          const detail = PARTNER_DETAILS[lookupKey] || {
            name: m.name,
            role: m.role,
            fourExpertise: ["Corporate Taxation", "Audit & Assurance", "GST Advisory", "Business Advisory"]
          }
          ...
          return (
            ...
            >
              ...
              <div className="h-[360px] w-full bg-slate-100 relative overflow-hidden">
                <img
                  src={m.image || "/images/team/placeholder.png"}
                  alt={detail.name}
              ...
              <div className="p-6 md:p-8 flex-1 flex flex-col">
                <h3 className="font-bold text-[22px] text-brand-dark">{detail.name}</h3>
          ...
          >
            ...
            <div className="w-full md:w-2/5 bg-slate-50 flex flex-col items-center p-8 border-b md:border-b-0 md:border-r border-slate-100">
              <div className="w-48 h-48 md:w-56 md:h-56 rounded-full overflow-hidden border-4 border-white shadow-lg relative mb-6">
                <img
                  src={selectedMember.image || "/images/team/placeholder.png"}
                  alt={selectedPartner.name}
                  className="w-full h-full object-cover select-none"
                />
              </div>

              <h3 className="font-bold text-[24px] text-brand-dark text-center">{selectedPartner.name}</h3>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="text-slate-600 text-[15px] leading-relaxed font-light space-y-4">
                    {selectedPartner.bio.map((para, idx) => (
                      <p key={idx}>{para}</p>
                    ))}
                  </div>
                </div>
